...
plot_L2078 <- function(res_obj) {
  ...
  # 分流逻辑 A：公式排行榜模式 (针对 Abs_Correlation)
  if ("Abs_Correlation" %in% colnames(res_df)) {
    p <- ggplot(utils::head(res_df, 12), 
      ...
      labs(title = "L2078: Formula Risk Ranking",
           ...
           x = "Mathematical Formula", y = "Correlation Score", fill = "Order") +
      theme_minimal()
    return(p)
  } 
  # 分流逻辑 B：共现气泡图模式 (针对 Strength)
    ...
    p <- ggplot(res_df, aes(x = Prevalence, y = Strength, color = as.factor(Order_L))) +
      ...
      labs(title = "L2078: Synergy Analysis", x = "Prevalence", y = "Strength") +
      theme_minimal()
    return(p)
  }
}
